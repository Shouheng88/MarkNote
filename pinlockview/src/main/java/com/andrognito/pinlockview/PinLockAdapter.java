...
public class PinLockAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final int VIEW_TYPE_FINGER = 2;

    private Context mContext;
    private CustomizationOptionsBundle mCustomizationOptionsBundle;
    private OnNumberClickListener mOnNumberClickListener;
    private OnDeleteClickListener mOnDeleteClickListener;
    private OnFingerClickListener mOnFingerClickListener;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_NUMBER) {
            ...
            View view = inflater.inflate(R.layout.layout_delete_item, parent, false);
            viewHolder = new FingerViewHolder(view);
        }
        return viewHolder;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        } else if (holder.getItemViewType() == VIEW_TYPE_FINGER) {
            FingerViewHolder vh3 = (FingerViewHolder) holder;
            configureFingerButtonHolder(vh3);
        }
    }
    ...
    private void configureFingerButtonHolder(FingerViewHolder holder) {
        if (holder != null) {
            if (mCustomizationOptionsBundle.isShowFingerButton()) {
                holder.mButtonImage.setVisibility(View.VISIBLE);
                if (mCustomizationOptionsBundle.getFingerButtonDrawable() != null) {
                    holder.mButtonImage.setImageDrawable(mCustomizationOptionsBundle.getFingerButtonDrawable());
                }
                holder.mButtonImage.setColorFilter(mCustomizationOptionsBundle.getTextColor(),
                        PorterDuff.Mode.SRC_ATOP);
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                        mCustomizationOptionsBundle.getFingerButtonSize(),
                        mCustomizationOptionsBundle.getFingerButtonSize());
                holder.mButtonImage.setLayoutParams(params);
            } else {
                holder.mButtonImage.setVisibility(View.GONE);
    ...
    public OnFingerClickListener getmOnFingerClickListener() {
        return mOnFingerClickListener;
    }

    public void setOnFingerClickListener(OnFingerClickListener mOnFingerClickListener) {
        this.mOnFingerClickListener = mOnFingerClickListener;
    }
    ...
    public class FingerViewHolder extends RecyclerView.ViewHolder {
        LinearLayout mDeleteButton;
        ImageView mButtonImage;

        public FingerViewHolder(final View itemView) {
            super(itemView);
            mDeleteButton = (LinearLayout) itemView.findViewById(R.id.button);
            mButtonImage = (ImageView) itemView.findViewById(R.id.buttonImage);

            if (mCustomizationOptionsBundle.isShowFingerButton() && mPinLength > 0) {
                mDeleteButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (mOnFingerClickListener != null) {
                            mOnFingerClickListener.onFingerClicked();
                        }
                    }
                });
